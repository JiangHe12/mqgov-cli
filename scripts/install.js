...
const EXPECTED_BINARIES = Object.freeze([
  'mqgov-cli-darwin-amd64',
  'mqgov-cli-darwin-arm64',
  'mqgov-cli-linux-amd64',
  'mqgov-cli-linux-arm64',
  'mqgov-cli-windows-amd64.exe',
  'mqgov-cli-windows-arm64.exe',
]);
...
function applyMirror(canonicalUrl) {
  const mirror = envWithDeprecatedAlias('MQGOV_DOWNLOAD_MIRROR', 'MQGOV_CLI_DOWNLOAD_MIRROR');
...
async function main() {
  ...
  const destination = path.join(destDir, os === 'windows' ? 'mqgov.exe' : 'mqgov');
