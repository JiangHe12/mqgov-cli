...
func TestClassifyGovernanceInvariants(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "dlq list R0", op: OperationListDLQ, want: safety.R0},
		{name: "dlq peek R0", op: OperationPeekDLQ, want: safety.R0},
		{name: "dlq redrive real execution pins R3 internal produce", op: OperationRedriveDLQ, target: Target{Topic: "orders"}, want: safety.R3},
		{name: "dlq redrive plan R0", op: OperationRedriveDLQ, target: Target{Topic: "orders", Plan: true}, want: safety.R0},
		{name: "mirror dry-run plan R0", op: OperationMirror, target: Target{Topic: "orders", Plan: true}, want: safety.R0},
		...
		{name: "dlq purge R3", op: OperationPurgeDLQ, target: Target{Topic: "orders-dlq"}, want: safety.R3},
		{name: "dlq purge plan R0", op: OperationPurgeDLQ, target: Target{Topic: "orders-dlq", Plan: true}, want: safety.R0},
...
func TestClassifyAdversarialEscalation(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "produce RocketMQ retry topic pins R3", op: OperationProduce, target: Target{Backend: "rocketmq", Topic: "%RETRY%billing"}, want: safety.R3},
		{name: "produce RocketMQ DLQ topic pins R3", op: OperationProduce, target: Target{Backend: "rocketmq", Topic: "%DLQ%billing"}, want: safety.R3},
		{name: "produce RocketMQ system topic pins R3", op: OperationProduce, target: Target{Backend: "rocketmq", Topic: "RMQ_SYS_TRANS_HALF_TOPIC"}, want: safety.R3},
		{name: "produce RocketMQ schedule topic pins R3", op: OperationProduce, target: Target{Backend: "rocketmq", Topic: "SCHEDULE_TOPIC_XXXX"}, want: safety.R3},
		{name: "same name is not internal on Kafka", op: OperationProduce, target: Target{Backend: "kafka", Topic: "%RETRY%billing"}, want: safety.R1},
		{name: "mirror internal target pins R3", op: OperationMirror, target: Target{Topic: "__consumer_offsets", InternalTopic: true}, want: safety.R3},
		{name: "mirror internal target dry-run is R0 preview", op: OperationMirror, target: Target{Topic: "__consumer_offsets", InternalTopic: true, Plan: true}, want: safety.R0},
		{name: "mirror wildcard target upgrades R1 to R2", op: OperationMirror, target: Target{Topic: "prod-*"}, want: safety.R2},
		...
		{name: "dlq redrive wildcard target pins R3", op: OperationRedriveDLQ, target: Target{Topic: "prod-*"}, want: safety.R3},
